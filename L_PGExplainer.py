...
class L_PGExplainer(nn.Module):
    ...
    def fit(self, G, epochs, output_directory: Path, experiment_info: dict, lr=0.01, loss_f=None, verbose=False):
        """train the MLP"""
        ...
        if loss_f is None:
            mal_weight = (positive_predictions == 0).sum() / (positive_predictions == 1).sum()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ner", "--edge-entropy-reg",   default=0.05, type=float)
    parser.add_argument("-nsr", "--edge-sum-reg",       default=0.05, type=float)
