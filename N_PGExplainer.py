...
class N_PGExplainer(nn.Module):
    ...
    def _sample_BCD_direct_weight(self, node_mask_logits, tau):
        ...
        logit = (
            torch.log2(epsilon)
            - torch.log2(1 - epsilon) + node_mask_logits
        ) / tau
        # logit = logit.clamp(-10, 10)  # prevent sigmoid overflow to nan
        return torch.sigmoid(logit)
    ...
    def approximate_subgraph_BCD_with_prior_masking(self, G, node_mask_logits, samples=30):
        ...
        near_binary_masks = []
        for _ in range(samples):
            near_binary_mask = self._sample_BCD_direct_weight(node_mask_logits, self.TAU)
            near_binary_masks.append(near_binary_mask)
            src, dst = G.edge_index
            # only disable edges not connected to any 'on' nodes
            corresponding_edge_mask = torch.max(near_binary_mask[src], near_binary_mask[dst])
            masked_y_pred, edge_embs, node_embs = self.model.forward(
    ...
    def fit(self, G, epochs, lr=0.01, loss_f=None, verbose=False, tb_logging_dir=None):
        ...
        for epc in tqdm(range(1, 1 + epochs)):
            ...
            masked_y_pred_mean_logits, average_near_binary_mask = self.approximate_subgraph_BCD_with_prior_masking(G, mask_logits)
            ...
            if tb_logging_dir:
                ...
                writer.add_scalar(f'all/mlp_first_layer_weight_grad', self.mlp[0].weight.grad.norm(), epc)
                writer.add_scalar(f'all/mlp_second_layer_weight_grad', self.mlp[2].weight.grad.norm(), epc)
            ...
            if tb_logging_dir:
                ...
                # scalars
                writer.add_scalar(f"all/Mean_Masked_Mal_Preds", (y_pred_masked > 0.5).float().sum(), epc)
                writer.add_scalar(f'all/Mean_preds_mean_value', torch.mean(y_pred_masked), epc)
                ...
                writer.add_scalar(f'all/Mean_value_for_BCD_near_binary_mask', torch.mean(average_near_binary_mask), epc)
                ...
                writer.add_histogram(f"Average_BCD_near_binary_mask", average_near_binary_mask, epc)
