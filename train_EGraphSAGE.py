...
if args.window is not None:
    logger.warning(
        f"using windowed mode for training and testing = [{args.window} flows per window]"
    )
    ...
    criterion = torch.nn.BCEWithLogitsLoss(pos_weight=torch.FloatTensor([mal_weight]))
