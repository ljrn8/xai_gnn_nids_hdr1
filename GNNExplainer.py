...
class GNNExplainer(nn.Module):
    ...
    def __init__(self, feature_bank, feature_reg_weight=0.1, edge_reg_weight=0.1, l1_norm_weight=0.1, **kwargs):
    ...
    def masked_forward(self, model, G, edge_mask, feature_mask, thresh=None, sample_Z=30):
...
def main(model, test_flows, epochs, window, lr):
    ...
    for i, (window_G, edge_W, feature_W, losses, regularization_penalties) in enumerate(gnne.fit(
        ...
        if sum(window_G.y == 1) > 100:
            ...
            explainability_report["average_reg_loss_curves"].append(regularization_penalties)
