class GNNExplainer(nn.Module):
    def __init__(self, feature_reg_weight, edge_reg_weight, **kwargs):
    ...
    def entropy_regularization(self, soft_mask):
        ...
        edge_mean_importances = torch.stack(
            [torch.mean(soft_mask[i, :]) for i in range(n_edges)]
        )
        edge_reg = self.edge_weight * torch.sum(
            edge_mean_importances * torch.log(edge_mean_importances)
        )
    ...
    def fit(
        ...
        for i, G in enumerate(yield_subgraphs(test_flows, window, linegraph=False)):
            ...
            learned_mask = nn.Parameter(
                torch.randn(G.edge_attr.shape).to(device), requires_grad=True
            )
